...
func (r *jobStatusResolver) resolve(ctx context.Context) map[int64]actions_model.Status {
	...
	for id, status := range r.statuses {
		...
		if err != nil {
			...
			// `if:` and let the runner evaluate the expression. Evaluating server-side can fail — most
			// often it PANICS inside the expression evaluator while resolving needs.<job_id>.outputs.*
			// references (evaluateJobIf recovers that panic and returns it as an error). Either way,
			// returning early left the job Blocked with nothing to retry it, so the run could stay
			// Blocked forever. Fall back to the pre-v1.27 behaviour: dispatch the job so the runner
			// evaluates `if:`. A transient/panicking evaluation then resolves on the runner, and a real
			// expression error fails the job on the runner instead of stranding the whole run.
